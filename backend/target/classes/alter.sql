...
-- ============================================================
-- GlamBook — Incremental Schema Migrations
...
ALTER TABLE reviews ADD CONSTRAINT fk_review_appointment FOREIGN KEY (appointment_id) REFERENCES appointments(id) ON DELETE SET NULL;
ALTER TABLE reviews ADD COLUMN photos JSON NULL;
ALTER TABLE reviews ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE';
ALTER TABLE reviews ADD COLUMN updated_at DATETIME NULL ON UPDATE CURRENT_TIMESTAMP;
...
ALTER TABLE consultations ADD COLUMN photo_url VARCHAR(500) NULL;
...
-- ================= PAYMENTS — Pay Later fields =================
ALTER TABLE payments ADD COLUMN pay_later_deadline DATETIME NULL;
ALTER TABLE payments ADD COLUMN pay_later_reminder_count INT NOT NULL DEFAULT 0;
ALTER TABLE payments MODIFY COLUMN status ENUM('PENDING','PAID','REFUNDED','PAY_LATER_PENDING') NOT NULL DEFAULT 'PENDING';
ALTER TABLE payments MODIFY COLUMN method ENUM('CASH','CARD','UPI','WALLET','PAY_LATER') NOT NULL DEFAULT 'UPI';
-- ================= CUSTOMER NOTIFICATIONS — add PAY_LATER_CONFIRMED =================
ALTER TABLE customer_notifications
MODIFY COLUMN type ENUM(
'BOOKING_CONFIRMED','BOOKING_CANCELLED',
'PAYMENT_SUCCESS','PAYMENT_REFUNDED',
